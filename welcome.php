...
function avatarka() {
    $userpic_dir = 'foto';
    $login = $_POST["login"];
    $name = $login . "_userpic_" . $_FILES["file"]["name"];
...
<body>
    ...
    <div>
        <?php
        if (isset($_SESSION['username']['0'])) {
            echo "Вы успешно авторизовались, " . $_SESSION['username']['0'];
        }
        ?>
    </div>
    <div>
        Последний раз вы посетили страницу:
        <?php
        if (isset($_COOKIE['LastPage'])) {
            echo $_COOKIE['LastPage'];
            echo "<br> <br>";
        } else {      
        echo "Вы не заходили на страницы bitrix.php или fact.php";
        echo "<br> <br>";
        }
        ?>
    </div>
    ...
    <div>
        <br>
        <a href="account.php">Перейти в личный кабинет</a>
    </div>
</body>
</html>